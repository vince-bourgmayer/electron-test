$(function()
{
    ...
    */
    ...
    $( "#filter-btn" ).on( 'click', function(){
        const valueOfFilter = $( "#name-filter-field" ).val()
        if( valueOfFilter.length == 0 )
            $( '#lockedDoors' ).slick( 'slickUnfilter' )
        else
            $( '#lockedDoors' ).slick( 'slickFilter', function(){
                return $(this).attr( 'id' ).match( valueOfFilter )
            })
        $( '#lockedDoors' ).slick( 'refresh' )        
    })
